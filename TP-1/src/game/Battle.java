...
public class Battle extends Game {
        ...
        switch (action) {
            ...
            case 4:
                if (selectedUnit.haveSuperAbility() && canAtack) {
                    Unit computerUnit = computerHero.getUnit(enemyCords);
                    useSuperAbility(selectedUnit, computerUnit, enemyCords, OwnerType.PERSON);
                    break;
                } else {
                    GameMenu.wrongChoose();
                }
    ...
    private void computerTurn() {
        ...
        if (computerUnit.haveSuperAbility()) {
            ...
            useSuperAbility(computerUnit, personUnit, enemyCoords, OwnerType.COMPUTER);
            return;
        }
        if (enemyCoords != null) {
    ...
    private void moveComputerUnit(Unit unit) {
        ...
        if (enemyCoords != null) {
            ...
            if (battleMap.isCellAvailable(newPos[0], newPos[1], false)) {
                battleMap.moveObject(new int[]{y, x}, newPos, unit.getOwner());
                unit.setPos(newPos);
            }
        } else {
    ...
    private void useSuperAbility(Unit recruiter, Unit recruit, int[] recruitCoords, OwnerType recruiterOwner) {
...
//                    GameMenu.println(recruitedNumber + " " + chance);
                    if (Math.random() < chance) {
                        if(recruiterOwner==OwnerType.PERSON) {
                            personHero.addUnit(recruit);
                            computerHero.kill(recruit);
                        }
                        else{
                            if(selectedUnit.equals(recruit)) selectedUnit = null;
                        ...
                        battleMap.reverseCellOwner(recruitCoords);
                    } else {
            ...
            System.out.println("У этого юнита нет суперспособностей.");
        }
    }
}